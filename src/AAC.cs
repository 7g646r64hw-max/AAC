...
===============================================================================
 ...
 MILESTONE 3.5.1 ENGINEERING CONSOLE POLISH
...
*/
    ...
    sealed class DisplayManager
    {
        ...
        void AppendConsoleStatusFields(StringBuilder b, PhysicsEngineModel pem, CapabilitySnapshot cap) { AppendField(b, "Status", AttentionStatus(pem), 10); AppendField(b, "Overall", pem.OverallHealth, 10); AppendField(b, "Axes READY", cap.ReadyAxisCount + "/6", 10); AppendField(b, "Warnings", WarningSummary(pem), 10); }
        void AppendDiscovery(StringBuilder b, PhysicsEngineModel p) { AppendField(b, "Gravity Generators", p.DetectedGeneratorCount + " / " + p.TaggedGravityGeneratorCount, 20); AppendField(b, "Artificial Mass", p.DetectedMassCount + " / " + p.TaggedArtificialMassCount, 20); }
        void AppendPem(StringBuilder b, PhysicsEngineModel p) { AppendField(b, "Reference Controller", ShortName(p.ReferenceControllerName, 24), 22); AppendField(b, "Frame Validity", p.CoordinateFrameValid ? "VALID" : "INVALID", 22); AppendField(b, "Generator Contrib.", p.ContributingGeneratorCount.ToString(), 22); AppendField(b, "Generator Non-Con.", (p.GravityGenerators.Count - p.ContributingGeneratorCount).ToString(), 22); AppendField(b, "Mass Contrib.", p.ContributingMassCount.ToString(), 22); AppendField(b, "Mass Non-Con.", (p.ArtificialMass.Count - p.ContributingMassCount).ToString(), 22); }
        void AppendCapability(StringBuilder b, PhysicsEngineModel p) { AxisCapability a = p.Axes[_debugManager.PageIndex]; AppendField(b, "State", a.Ready ? "READY" : "NOT READY", 11); AppendField(b, "Reason", CapabilityReason(p, a), 11); AppendField(b, "Generators", a.GeneratorCount.ToString(), 11); AppendField(b, "Tolerance", a.Tolerance.ToString(), 11); }
        void AppendGenerator(StringBuilder b, List<PemBlock> blocks, int index) { PemBlock x; if (!TryBlock(b, blocks, ref index, "No gravity generators.")) return; x = blocks[index]; AppendField(b, "Entity ID", x.EntityId.ToString(), 13); AppendField(b, "Name", ShortName(x.CustomName, 24), 13); AppendField(b, "Orientation", x.Orientation, 13); AppendField(b, "Gravity Axis", x.GravityAxis, 13); AppendField(b, "Distance", x.DistanceText, 13); AppendField(b, "Enabled", YesNo(x.Enabled), 13); AppendField(b, "Working", YesNo(x.Working), 13); AppendField(b, "Validated", x.Validation, 13); AppendField(b, "Contributing", YesNo(x.Contributing), 13); }
        ...
        void AppendPerformance(StringBuilder b, PerformanceMonitor p) { AppendField(b, "Last Scan", FormatMs(p.LastScanMs), 28); AppendField(b, "Average Scan", FormatMs(p.AverageScanMs), 28); AppendField(b, "PEM Build Time", FormatMs(p.PemBuildMs), 28); AppendField(b, "Capability Assessment Time", FormatMs(p.CapabilityMs), 28); }
        ...
        static void AppendField(StringBuilder b, string label, string value, int width) { b.Append(label.PadRight(width)).Append(": ").AppendLine(value); }
